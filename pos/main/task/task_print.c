...
static const char *TAG = "PrintTask";
extern QueueHandle_t print_que;
...
static void task(void *parameter)
{
  while (1)
  {
    ...
    // Check if there is a message in the print_que queue
    if (print_que != NULL && uxQueueMessagesWaiting(print_que) > 0)
    {
      // Allocate memory for the message structure
      Message *msg = (Message *)mem_alloc(sizeof(Message));
      if (msg != NULL)
      {
        // Receive the message from the print_que queue
        if (xQueueReceive(print_que, msg, 0) == pdPASS)
        {
