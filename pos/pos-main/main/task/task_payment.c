...
static const char *TAG = "PaymentTask";
extern QueueHandle_t payment_que;
...
static void task(void *parameter)
{
  while (1)
  {
    ...
    // Check if there is a message in the payment_que queue
    if (payment_que != NULL && uxQueueMessagesWaiting(payment_que) > 0)
    {
      // Allocate memory for the message structure
      Message *msg = (Message *)mem_alloc(sizeof(Message));
      if (msg != NULL)
      {
        // Receive the message from the payment_que queue
        if (xQueueReceive(payment_que, msg, 0) == pdPASS)
        {
